...
def generateProblem(client, numExamplesToPull=3):
    ...
    system_prompt = (
        ...
        # "After generating the problem, please retrace your steps and tell me what mathematical concepts you used to create the problem. Do not include any others words besides the math concepts"
        #  "After generating the problem, please retrace your steps and using MathLib Library tell me what mathematical theorem you used to create the problem. Do not include any others words besides the math theorems"
...
def identify_key_vars(client, code):
    print("Identifying Key Variables \n")
    response = client.chat.completions.create(
        model = "gpt-4",
        messages=[
            {"role": "system", "content": (
                "You are a Math Genius. Your task is to take the problem and identify all key variables used in the problem"
                "Do NOT include any additional comments or explanation-output, just the key variables"
            )},
...
def create_system_equations(client, code):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": (
                "You are a Math Genius. Your task is to take the problem and generate a linear system of equations out of it"
                "First identify all key variables, including the one that you would need to solve for."
                "From there, create a linear systems of equations."
...
# determine_uniqueness(create_Matrix(client,create_system_equations(client, generateProblem(client))))
create_system_equations(client, generateProblem(client))
